...
/* Class take from https://github.com/chiralcode/Android-Color-Picker  */
...
public class ColorPicker extends View {
	 ...
	 */
	private final int paramOuterPadding = 2; // outer padding of the whole color
												// picker view
	private final int paramInnerPadding = 5; // distance between value slider
												// wheel and inner color wheel
	private final int paramValueSliderWidth = 10; // width of the value slider
	private final int paramArrowPointerSize = 4; // size of the arrow pointer;
													// set to 0 to hide the
													// pointer
		...
		case MotionEvent.ACTION_MOVE:
			...
			if (d <= colorWheelRadius) {
				...
				colorHSV[2] = (float) Math.max(0,
						Math.min(1, Math.atan2(cy, cx) / Math.PI + 0.5f));
