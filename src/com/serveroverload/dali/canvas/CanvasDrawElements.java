...
public class CanvasDrawElements extends View {
	...
	private void drawText(Canvas canvas) {
		...
		int numChars = (lengthOfChar <= 0) ? 1 : (int) Math
				.floor((double) (restWidth / lengthOfChar)); // The number of
																// characters at
																// 1 line
	...
	private void onActionMove(MotionEvent event) {
		...
		case ERASER:
			...
			if ((this.drawer != Drawer.QUADRATIC_BEZIER)
				...
				case RECTANGLE:
					path.reset();
					path.addRect(this.startX, this.startY, x, y,
							Path.Direction.CCW);
					break;
				case CIRCLE:
					double distanceX = Math.abs((double) (this.startX - x));
					double distanceY = Math.abs((double) (this.startX - y));
					double radius = Math.sqrt(Math.pow(distanceX, 2.0)
							+ Math.pow(distanceY, 2.0));
	...
	public void drawBitmap(byte[] byteArray) {
		this.drawBitmap(BitmapFactory.decodeByteArray(byteArray, 0,
				byteArray.length));
	}
